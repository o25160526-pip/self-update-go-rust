...
The docx skill is located at `.skills/docx/` inside the workspace. Use `CURRENT_SKILL_PATH` from `<runtime_config>` if available.
```bash
# Skill root is at workspace/.skills/docx
find .skills -name "document.py" -path "*/docx/scripts/*" 2>/dev/null | head -1
```

**Run your script with PYTHONPATH** set to the docx skill root:
```bash
PYTHONPATH=.skills/docx python your_script.py
```
